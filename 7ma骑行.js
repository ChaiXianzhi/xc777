/**
 作者QQ:2993411782
 7MA出行 每日签到、看广告得积分  
 每天18积分，90积分换一次电动车助力，188积分可换3次免费骑行
 
 抓包：7MA出行 微信小程序或者app都可以（建议app，app可用小黄鸟直接抓。小程序高版本安卓需要虚拟机抓包）
 抓 newmapi.7mate.cn 这个域名下 hearders 部分的 Authorization ,
 把抓到的值去掉前面的Bearer，只保留 eyJ0eXAiOiJKxxxxxxxxxxx 后面这部分
        ...
        $.get(url, async (error, response, data) => {
            try {
                ...
                if(result.data.phone) {
                    login_log =`HPUer，登录成功\n` + `手机：` + result.data.phone + `\nID：` + result.data.id + `\n积分：` + result.data.points + `\n学校：` + result.data.school_name
